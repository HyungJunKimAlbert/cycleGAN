...
def test(args):
    ...
    if mode == 'test':
        ...
        with torch.no_grad():
            netG_a2b.eval()    # Generator
            netG_b2a.eval()
            netD_a.eval()    # Discriminator
            netD_b.eval()
            ...
            for batch, data in enumerate(loader_test_a, 1):
                ...
                for j in range(input_a.shape[0]):
                    ...
                    plt.imsave(os.path.join(result_dir_test, 'png', '%04d_input_a.png' % id), input_a_.squeeze(), cmap=cmap)
                    plt.imsave(os.path.join(result_dir_test, 'png', '%04d_output_b.png' % id), output_b_.squeeze(), cmap=cmap)

                    print(f"TEST A: BATCH {id + 1} / { num_data_test_a } ")
            ...
            for batch, data in enumerate(loader_test_b, 1):
                ...
                for j in range(input_b.shape[0]):
                    ...
                    plt.imsave(os.path.join(result_dir_test, 'png', '%04d_input_b.png' % id), input_b_.squeeze(), cmap=cmap)
                    plt.imsave(os.path.join(result_dir_test, 'png', '%04d_output_a.png' % id), output_a_.squeeze(), cmap=cmap)

                    print(f"TEST B: BATCH {id + 1} / { num_data_test_b } ")
